...
RegisterNetEvent("hs_moneywash:server:checkforbills")
AddEventHandler("hs_moneywash:server:checkforbills", function()
    ...
    for slot, data in pairs(Player.PlayerData.items) do
        ...
        if ServerDataWorth > 0 and amount > 0 then
            ...
            TriggerClientEvent('hs_moneywash:client:exchangebills', src, ServerDataWorth)
...
RegisterNetEvent("hs_moneywash:server:returncleancash")
AddEventHandler("hs_moneywash:server:returncleancash", function(ServerDataWorth)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
