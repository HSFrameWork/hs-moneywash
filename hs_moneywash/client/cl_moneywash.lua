    ...
    if not DoesEntityExist(dirtydan) then
        ...
        exports['qb-target']:AddTargetEntity(dirtydan, {
            options = {
                {
                    type = "server",
                    event = "hs_moneywash:server:checkforbills",
...
RegisterNetEvent('hs_moneywash:client:exchangebills')
AddEventHandler('hs_moneywash:client:exchangebills', function(ServerDataWorth)
    ...
    QBCore.Functions.Progressbar("cleanbills", "Exchanging marked bills..", 10000, false, false, {
        ...
        TriggerServerEvent("hs_moneywash:server:returncleancash", ServerDataWorth)
    end)
end)
